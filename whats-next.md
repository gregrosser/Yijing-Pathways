...
**Verification of the Jekyll work without Jekyll:** simulated the pipeline in Python — stripped front matter, converted the body with `pandoc -f markdown-raw_tex -t html5 --wrap=none`, substituted the Liquid variables and the strand conditional (the `if page.strand == 'Culture'` branch that swaps the accent colour), wrote the result, confirmed zero unresolved Liquid tags, and rendered it in headless Chromium. Both strand accents resolved correctly and the raw HTML tables inside the notes survived intact.
...
- **Never put raw Liquid delimiters in a tracked markdown file.** GitHub Pages enables `jekyll-optional-front-matter` by default, so markdown *without* front matter is still rendered as a page and Liquid-parsed. An unmatched tag is a build error, and a build error means the site silently keeps serving the previous deploy. This was hit for real: the first version of this handoff quoted a layout conditional literally, which was the only Liquid in any tracked `.md`. Check with `git ls-files '*.md' | xargs grep -nE '\{[%{]'` before pushing prose that discusses templates — that pattern matches both delimiters without containing either, so the check itself is safe to commit.
